...
function ComedyMovieConnexion() {
  const [Data, SetData] = useState<{ results: MovieData[] }>({ results: [] });
  const [Loading, SetLoading] = useState<boolean>(true);
  const apiKey = "dde133e5dad4ecdfe125539fc3db123d";

  useEffect(() => {
    const fetchComedyMovieData = async () => {
      try {
        const response = await fetch(`https://api.themoviedb.org/3/discover/movie?api_key=${apiKey}&with_genres=35`);
        const data = await response.json();
        SetData(data);
      } catch (error) {
        console.log("Une erreur est intervenue lors de la récupération des données concernant les films de comédie", error);
      } finally {
        SetLoading(false);
      }
    };
    fetchComedyMovieData();
  }, []);
...
export default ComedyMovieConnexion;
